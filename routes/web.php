...
Route::prefix("admin")->as("backend.admin.")->middleware(["admin"])->group(function () {
    ...
    // ШАГ 1 FIX-1: /clear-all и /storage-link — перенесены под middleware admin
    // Риск: любой неавторизованный пользователь мог очистить кеш или создать symlink
    Route::get("clear-cache", function () {
        \Illuminate\Support\Facades\Artisan::call("optimize:clear");
        return redirect()->back()->with("success", "Cache cleared.");
    })->name("clear.cache")->middleware("can:settings_manage");

    Route::get("storage-link", function () {
        \Illuminate\Support\Facades\Artisan::call("storage:link");
        return redirect()->back()->with("success", "Storage linked.");
    })->name("storage.link")->middleware("can:settings_manage");
});
